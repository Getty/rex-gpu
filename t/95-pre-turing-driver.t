...
#
# Everything under test is pure (hash/string only, no run/dpkg/rpm):
#   * Rex::GPU::Detect::legacy_driver_requirement — device-ID ranges
#   * Rex::GPU::NVIDIA::_reject_unsupported_legacy_gpu — the Kepler die
#   * _ubuntu_legacy_driver_package, _rhel_legacy_driver_plan,
#     _suse_nvidia_repo_params($release, $legacy) — per-distro selection
#   * _apt_candidate_present, _rpm_version_in_branch — verify predicates
# The non-regression claim: Turing-and-later and unknown IDs get undef /
# the unchanged default from every selector.
#
...
subtest 'real lspci line => device id => requirement' => sub {
  ...
  is(Rex::GPU::NVIDIA::_legacy_driver_requirement($g)->{max_branch}, 580, 'V100 => 580');
};
...
subtest 'Ubuntu: pinned proprietary 580 -server for pre-Turing only' => sub {
  my $pkg = \&Rex::GPU::NVIDIA::_ubuntu_legacy_driver_package;
  is(scalar $pkg->($v100), 'nvidia-driver-580-server', 'V100 => nvidia-driver-580-server');
  is(scalar $pkg->($p100), 'nvidia-driver-580-server', 'P100 => nvidia-driver-580-server');
  is(scalar $pkg->($_), undef, "$_->{device_id} => undef (newest -server path)")
    for $t4, $rtx4000, $b200;
  is(scalar $pkg->(undef), undef, 'no GPU => undef');
};

subtest 'RHEL: stream on 8/9, versionlock on 10, proprietary kmod' => sub {
  my $plan = \&Rex::GPU::NVIDIA::_rhel_legacy_driver_plan;
  for my $major (8, 9) {
    ...
    is($p->{kmod}, 'kmod-nvidia-latest-dkms', "RHEL $major => verifies proprietary kmod");
  }
  my $p = $plan->(10, $v100);
  is($p->{module_stream}, undef,          'RHEL 10 => no module stream');
  is($p->{versionlock},   '*nvidia*580*', 'RHEL 10 => versionlock *nvidia*580*');
  is($p->{branch},        580,            'branch 580');
  is(scalar $plan->($_->[0], $_->[1]), undef, "RHEL $_->[0] + " . ($_->[1] ? $_->[1]{device_id} : "no GPU") . " => undef (open path)")
    for [9, $t4], [10, $rtx4000], [10, $b200], [9, undef];
};

subtest 'SUSE: proprietary G06 for pre-Turing, default otherwise' => sub {
  my $params = \&Rex::GPU::NVIDIA::_suse_nvidia_repo_params;
  my $legacy = Rex::GPU::NVIDIA::_legacy_driver_requirement($v100);
  is_deeply([ $params->('15.6', $legacy) ],
    [ 'https://download.nvidia.com/opensuse/leap/15.6/', 'nvidia-driver-G06-kmp-meta' ],
    'Leap 15.6 + V100 => leap/15.6 + proprietary G06');
  is_deeply([ $params->('16.0', $legacy) ],
